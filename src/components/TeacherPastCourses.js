...
const TeacherPastCourses = ({ user, courses, feedbacks }) => {
    ...
    filteredCourses.forEach(course => {
        cumulative = cumulative + parseInt(course.points);
    });
    ...
    return (
        <>
            <div className="completed-container">
                <h3>Pitämäsi kurssit</h3>
                <p>Löydät kaikki jo päättyneet pitämäsi kurssisi tältä listalta.</p>
                <div className="completion-counter">{cumulative}</div>
                {filteredCourses.length === 0 && <div className="noCourses">Yksikään pitämäsi kurssi ei ole vielä päättynyt.</div>}
